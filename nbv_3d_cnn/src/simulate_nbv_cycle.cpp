...
class SimulateNBVCycle
{
    ...
    m_nh(nh), m_opencl(m_nh)
  {
    ...
    m_random_engine.seed(param_int);
    ...
    m_nh.param<int>(PARAM_NAME_SENSOR_RESOLUTION_DOWNSAMPLE, param_int, PARAM_DEFAULT_SENSOR_RESOLUTION_DOWNSAMPLE);
    m_sensor_resolution_downsample = param_int;
    ...
    bool use_octree_for_nbv;
    m_nh.param<bool>("use_octree_for_nbv", use_octree_for_nbv, false);
    ...
    if (param_string == PARAM_VALUE_NBV_ALGORITHM_RANDOM)
      ...
      m_current_adapter.reset(new AutocompleteIGainNBVAdapter(m_nh, m_opencl, *m_generate_single_image,
                                                              ...
                                                              m_accuracy_skip_voxels * m_cnn_accuracy_skip_voxels,
                                                              0));
    else if (param_string == PARAM_VALUE_NBV_ALGORITHM_AutocompleteOctreeIGain)
      m_current_adapter.reset(new AutocompleteOctreeIGainNBVAdapter(m_nh, m_opencl, *m_generate_single_image, m_random_engine,
                                                                    m_sensor_range_voxels,
                                                                    m_sensor_min_range_voxels,
                                                                    Eigen::Vector2i(m_sensor_resolution_x / m_sensor_resolution_downsample,
                                                                                    m_is_3d ? (m_sensor_resolution_y /
                                                                                               m_sensor_resolution_downsample) : 1),
                                                                    m_sensor_focal_length / m_sensor_resolution_downsample,
                                                                    true,
                                                                    m_is_3d, true, use_octree_for_nbv,
                                                                    m_accuracy_skip_voxels * m_cnn_accuracy_skip_voxels,
                                                                    m_sample_fixed_number_of_views));
    else if (param_string == PARAM_VALUE_NBV_ALGORITHM_AutocompleteVoxelgridIGain)
      m_current_adapter.reset(new AutocompleteOctreeIGainNBVAdapter(m_nh, m_opencl, *m_generate_single_image, m_random_engine,
                                                                    m_sensor_range_voxels,
                                                                    m_sensor_min_range_voxels,
                                                                    Eigen::Vector2i(m_sensor_resolution_x / m_sensor_resolution_downsample,
                                                                    ...
                                                                    m_sensor_focal_length / m_sensor_resolution_downsample,
                                                                    true,
                                                                    m_is_3d, false, use_octree_for_nbv,
                                                                    m_accuracy_skip_voxels * m_cnn_accuracy_skip_voxels,
                                                                    m_sample_fixed_number_of_views));
  ...
  Eigen::Quaternionf RandomOrientation()
  {
    if (!m_is_3d)
    {
      const float orient = std::uniform_real_distribution<float>(0.0f, 1.0f)(m_random_engine) * M_PI * 2.0f;
    ...
    {
      const Eigen::Vector3f random_axis = Eigen::Vector3f(
            std::uniform_real_distribution<float>(-1.0f, 1.0f)(m_random_engine),
            std::uniform_real_distribution<float>(-1.0f, 1.0f)(m_random_engine),
            std::uniform_real_distribution<float>(-1.0f, 1.0f)(m_random_engine)
            ).normalized();
      const float random_angle = std::uniform_real_distribution<float>(0.0f, 1.0f)(m_random_engine) * M_PI * 2.0f;
      return Eigen::Quaternionf(Eigen::AngleAxisf(random_angle, random_axis));
    }
  }
  ...
  Eigen::Vector3f FindAvailableOrigin(const Voxelgrid & environment)
  {
    ...
    const uint64 selected = std::uniform_int_distribution<uint64>(0, count)(m_random_engine);
    count = 0;
  ...
  void onTimer(const ros::TimerEvent &)
  {
    ...
    else
    {
      ...
      ROS_INFO("simulate_nbv_cycle: nbv computation time: %f s", float(computation_time));
      ...
      if (m_save_images)
      {
        ...
        m_current_adapter->SaveDebugGrids(test_folder + std::to_string(m_loop_counter) + "_", m_environment_image);
      }
    }
...
//                                                                                 occupied);
    ...
    AppendToLog(m_loop_counter, m_environment_image, m_current_observed_empty, computation_time,
                max_origin, max_orientation, m_current_adapter->GetDebugInfo());
  ...
  void AppendToLog(const uint64 loop_counter, const Voxelgrid & environment,
                   ...
                   const Eigen::Vector3f & position, const Eigen::Quaternionf & rotation,
                   const StringStringMap & debug_info)
  {
...

    {
      ...
      header_str << "Iteration" << "\t" << "\"Current unknown\"" << "\t" << "\"Total unknown\"" << "\t" <<
               "\"Computation time\"";
      for (StringStringPair di : debug_info)
        header_str << "\t\"" << di.first << "\"";
      header_str << "\tPosition_x" << "\t" << "\"Position_y\"" << "\t" << "\"Position_z\"" << "\t" <<
                    "Rotation_x" << "\t" << "\"Rotation_y\"" << "\t" << "\"Rotation_z\"" << "\t" << "\"Rotation_w\"";
      header_str << "\n";
      const std::string header = header_str.str();
      if (m_prev_log_header != header) // if header changed, re-save it
      {
        ofile << header;
        m_prev_log_header = header;
      }
    }
    ...
    ofile << rotation.x() << "\t" << rotation.y() << "\t" << rotation.z() << "\t" << rotation.w() << "\n";
  ...
  uint64 m_sensor_resolution_downsample;
