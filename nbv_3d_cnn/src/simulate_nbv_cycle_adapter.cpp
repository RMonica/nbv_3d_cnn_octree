                                                ...
                                                ViewWithScoreVector * const all_views_with_scores)
{
  ...
  ROS_INFO("nbv_3d_cnn: InformationGainOctreeNBVAdapter: computing origins");

  Eigen::Vector3i roi_min = Eigen::Vector3i::Zero();
  Eigen::Vector3i roi_max = Eigen::Vector3i(width, height, depth);
  ROS_INFO_STREAM("nbv_3d_cnn: InformationGainOctreeNBVAdapter: ROI is: "
  ...
  m_last_debug_info.insert(StringStringPair("opencl_raycast_time", std::to_string(m_generate_single_image.GetLastOpenCLComputationTime())));
  m_last_debug_info.insert(StringStringPair("opencl_raycast_num_rays",
                                            std::to_string(m_generate_single_image.GetLastNumberOfRays())));
  m_last_debug_info.insert(StringStringPair("opencl_raycast_simulation_time", std::to_string(m_generate_single_image.GetLastSimulationTime())));
  m_last_debug_info.insert(StringStringPair("opencl_raycast_upload_time", std::to_string(m_generate_single_image.GetLastUploadTime())));
...
bool AutocompleteIGainNBVAdapter::Predict3d(const Voxelgrid &empty, const Voxelgrid &frontier, Voxelgrid &autocompleted)
{
  ...
  m_last_debug_info.clear();
  for (nbv_3d_cnn_msgs::DebugInfo di : result.debug_info)
    m_last_debug_info[di.name] = di.value;
                                                  ...
                                                  ViewWithScoreVector * const all_views_with_scores)
{
  ...
  if (m_is_3d)
  {
    if (!Predict3d(empty, frontier, autocompleted))
      return false;
  }
  else
  {
    if (!Predict(empty, frontier, autocompleted))
      return false;
  }
