...
class Generate3DScenesRealistic
{
  ...
  Generate3DScenesRealistic(ros::NodeHandle & nh): m_nh(nh)
  {
    ...
    m_nh.param<int>(PARAM_NAME_INITIAL_SCENE_NUM, param_int, PARAM_DEFAULT_INITIAL_SCENE_NUM);
    m_initial_scene_num = param_int;

    LoadObjects();

    m_scene_counter = m_initial_scene_num;
    m_timer = m_nh.createTimer(ros::Duration(0.0), &Generate3DScenesRealistic::onTimer, this, true);
  }

  Uint64Vector ParamToIndexVector(const std::string & param)
  {
  ...
  void LoadObjects()
  {
    ...
    for (uint64 i = 0; i < m_object_folder_list.size(); i++)
    {
      ...
      std::string explicit_str;
      m_nh.param<std::string>(std::string(PARAM_NAME_INDICES_EXPLICIT) +
                              std::to_string(i), explicit_str, PARAM_DEFAULT_INDICES_EXPLICIT);
      const bool is_explicit = (explicit_str != PARAM_DEFAULT_INDICES_EXPLICIT);
      Uint64Vector explicit_indices;
      if (is_explicit)
      {
        explicit_indices = ParamToIndexVector(explicit_str);
        ROS_INFO("generate_3d_scenes:   explicit indices: \"%s\"", explicit_str.c_str());
      }
      ...
      while (true)
      {
        ...
        uint64 file_index = file_counter + 1;
        if (is_explicit)
          file_index = explicit_indices[file_counter];

        ofilename << std::setfill('0') << std::setw(2) << file_index << std::flush;
        ...
        ROS_INFO("generate_3d_scenes:   success");
        m_object_point_clouds.push_back(cloud);
        file_counter++;

        if (is_explicit)
          if (file_counter >= explicit_indices.size())
            break;
      }
    }
